...
data = truth_train
_ = model(data[:batch_size], t) # Test the model with a small batch to see if it works
...
seeds = 1000
probe_data = torch.zeros((seeds, timesteps, 1, numchannels, Nx, Ny))
model.eval()
...
## See the code in ddpm_turb2d_test.py for reverse sampling
...
timesteps_rev_list = np.arange(stimesteps, ftimesteps)
...
# with open(f"{model_dir}/probe_data_seeds-{seeds}_sts-{stimesteps}_fts-{ftimesteps}.pkl", "wb") as f:
#     pickle.dump(probe_data, f)

# with open(f"{model_dir}/probe_data_seeds-{seeds}_sts-{stimesteps}_fts-{ftimesteps}.pkl", "rb") as f:
#     probe_data = pickle.load(f)

## test plots
# path_outputs_model = f"{output_dir}/{model_name}"
path_outputs_model = f"{output_dir}/{model_name}_mod_lastTstepBeta"
...
for revfinal in [len(its_timesteps)]:
    ...
    cbar.set_ticklabels([f"{int(t)}" for t in np.linspace(ftimesteps, stimesteps, num=numticks)])
    ...
    plt.suptitle(f"Probe data FFT\nReverse timesteps from {ftimesteps} to {stimesteps}")
...
for revfinal in [itrevfinal, len(its_timesteps)]:
    for yscale in ["linear", "log"]:
        ...
        plt.suptitle(f"Probe data FFT\nReverse timesteps from {ftimesteps} to {stimesteps}, repeated beta")
        plt.savefig(f"{path_outputs_model}/spectrum_revFinal-{revfinal}_yscale-{yscale}.png", dpi = 200)
"""